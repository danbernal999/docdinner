...
if (isset($_POST['ver_total_categoria'])) {
    ...
    $stmt->execute([':categoria' => $categoriaSeleccionada]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $total_categoria = $result['total_categoria'] ?? 0;
}

// Manejar el orden por fecha o monto
if (isset($_POST['ordenar_por'])) {
    $ordenSeleccionado = $_POST['orden'];
    $sql = "SELECT * FROM gastos_fijos ORDER BY " . ($ordenSeleccionado === 'fecha' ? 'fecha' : 'monto') . " DESC";
    $stmt = $conn->query($sql);
    $gastos = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
?>
...
<body class="bg-neutral-50 min-h-screen text-neutral-950 p-4 md:p-8">
  <!-- Encabezado -->
  <header class="mb-6">
