<div class="bg-neutral-50 min-h-screen text-neutral-950 p-4 md:p-8">
                    ...
                    <form action="index.php?ruta=main&modulo=productos" method="POST" class="p-4">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="flex items-center gap-3">
                                <input type="checkbox" id="es_recurrente_edit_<?= $gasto['id'] ?>" name="es_recurrente" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded" <?= $gasto['es_recurrente'] ? 'checked' : '' ?>>
                                <label for="es_recurrente_edit_<?= $gasto['id'] ?>" class="text-sm text-gray-700">Sí, es un gasto recurrente</label>
                            </div>
                        </div>

                        <div id="recurrencia-section-edit-<?= $gasto['id'] ?>" class="mb-4 <?= $gasto['es_recurrente'] ? '' : 'hidden' ?>">
                            <label for="frecuencia_recurrencia_edit_<?= $gasto['id'] ?>" class="block mb-2 text-sm font-medium text-gray-900">Frecuencia de Recurrencia</label>
                            <select id="frecuencia_recurrencia_edit_<?= $gasto['id'] ?>" name="frecuencia_recurrencia" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                                <option value="">Seleccione la frecuencia</option>
                                <?php
                                $frecuencias = ['diario', 'semanal', 'quincenal', 'mensual', 'trimestral', 'semestral', 'anual'];
                                foreach ($frecuencias as $frec) {
                                    $selected = ($gasto['frecuencia_recurrencia'] == $frec) ? 'selected' : '';
                                    echo "<option value=\"" . htmlspecialchars($frec) . "\" $selected>" . htmlspecialchars(ucfirst($frec)) . "</option>";
                                }
                                ?>
                            </select>
                        </div>
                        <script>
                            document.addEventListener('DOMContentLoaded', function() {
                                ...
                                const recurrenciaSectionEdit = document.getElementById('recurrencia-section-edit-<?= $gasto['id'] ?>');
                                const frecuenciaRecurrenciaSelectEdit = document.getElementById('frecuencia_recurrencia_edit_<?= $gasto['id'] ?>');

                                esRecurrenteCheckboxEdit.addEventListener('change', function() {
                                    if (this.checked) {
                                        recurrenciaSectionEdit.classList.remove('hidden');
                                        frecuenciaRecurrenciaSelectEdit.setAttribute('required', 'required');
                                    } else {
                                        recurrenciaSectionEdit.classList.add('hidden');
                                        frecuenciaRecurrenciaSelectEdit.removeAttribute('required');
                                        frecuenciaRecurrenciaSelectEdit.value = ''; // Resetear la selección
                                    }
                                });
                            });
                        </script>
            ...
            <div class="relative bg-white rounded-lg shadow">
                ...
                <form action="index.php?ruta=main&modulo=productos" method="POST" class="p-4">
                    ...
                    <div class="mb-4">
                        <label class="block mb-2 text-sm font-medium text-gray-900">¿Es un gasto recurrente?</label>
                        <div class="flex items-center gap-3">
                            <input type="checkbox" id="es_recurrente" name="es_recurrente" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded">
                            <label for="es_recurrente" class="text-sm text-gray-700">Sí, es un gasto recurrente</label>
                        </div>
                    </div>

                    <div id="recurrencia-section" class="mb-4 hidden">
                        <label for="frecuencia_recurrencia" class="block mb-2 text-sm font-medium text-gray-900">Frecuencia de Recurrencia</label>
                        <select id="frecuencia_recurrencia" name="frecuencia_recurrencia" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                            <option value="">Seleccione la frecuencia</option>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const recurrenciaSection = document.getElementById('recurrencia-section');
            const frecuenciaRecurrenciaSelect = document.getElementById('frecuencia_recurrencia');

            esRecurrenteCheckbox.addEventListener('change', function() {
                if (this.checked) {
                    recurrenciaSection.classList.remove('hidden');
                    frecuenciaRecurrenciaSelect.setAttribute('required', 'required');
                } else {
                    recurrenciaSection.classList.add('hidden');
                    frecuenciaRecurrenciaSelect.removeAttribute('required');
                    frecuenciaRecurrenciaSelect.value = ''; // Resetear la selección
                }
            });

            // Asegurarse de que el estado inicial se maneje correctamente si el checkbox está marcado por defecto
